module.exports.config = {
  name: "paste2",
  des: "Paste A Non Prefix Command Code",
  nodeDepends:{},
  author: "D-Jukie",
  version: "1.0.0",
  perm: 3
...
module.exports.start = async function ({ event, api }) {
  ...
  if(!text && name) {
    var data = fs.readFile(
      `${__dirname}/../noPrefix/${args[0]}.js`,
      "utf-8",
      async (err, data) => {
        if (err) return api.sendMessage(`Command ${args[0]} does not exist!.`, threadID, messageID);
        const { PasteClient } = require('pastebin-api');
        const client = new PasteClient("P22zrRZoMdO5GFTJ3tmUqmYC6QPchHer");
  ...
  if (url[0].indexOf('pastebin') !== -1) {
    axios.get(url[0]).then(i => {
      var data = i.data;
      fs.writeFile(`${__dirname}/../noPrefix/${args[0]}.js`, data, "utf-8", function (err) {
        if (err) return api.sendMessage(`An error occurred while applying the code ${args[0]}.js`, threadID, messageID);
        api.sendMessage(`Applied the code to ${args[0]}.js, restart the program to use!`, threadID, messageID);
      }
    );
})
  }
  if (url[0].indexOf('buildtool') !== -1 || url[0].indexOf('tinyurl.com') !== -1) {
    ...
    request(options, function (error, response, body) {
      ...
      load('.language-js').each((index, el) => {
        ...
        fs.writeFile(`${__dirname}/../noPrefix/${args[0]}.js`, code, "utf-8",
                     function (err) {
                       if (err) return api.sendMessage(`An error occurred while applying the new code to "${args[0]}.js".`, threadID, messageID);
                       return api.sendMessage(`Added this code "${args[0]}.js", restart the bot to use it!`, threadID, messageID);
                     }
);
});
});
